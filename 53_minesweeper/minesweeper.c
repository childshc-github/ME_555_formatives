...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  board_t * game = malloc(sizeof(*game));
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  int mine_count = 0;
  int max_height = b->height;
  int max_width = b->width;

  // check via loop - don't count given
  for (int vert = (y - 1); vert <= (y + 1); vert++) {
    for (int horz = (x - 1); horz <= (x + 1); horz++) {
      if ((vert >= max_height) || (vert < 0)) {
        continue;
      }
      if ((horz >= max_width) || (horz < 0)) {
        continue;
      }
      if ((vert == y) && (horz == x)) {
        continue;
      }
      if (IS_MINE(b->board[vert][horz])) {
        mine_count++;
      }
    }
  }
